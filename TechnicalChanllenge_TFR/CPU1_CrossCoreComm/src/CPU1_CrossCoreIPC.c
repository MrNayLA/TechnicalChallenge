...
uint8_t CPU1_TestLockStatus = 1;//Used for mock test Only
...
uint8_t CPU1_Acquire_IPC_Lock(uint8_t ChannelNo)
{
	...
	LockStatus = CPU1_TestLockStatus;//Used for mock test Only
...
void CPU1_ISR_from_IPC_Channel0(void)
{
...
void CPU1_ISR_from_IPC_Channel1(void)
{
	if(1 /* if notify status is set in the HW register. */)
	{
		u8Channel1_Noti = CPU1_Check_IPC_Notification(1);//Acknowledge
	}
...
void CPU1_ISR_from_IPC_Channel2(void)
{
	if(1 /* if notify status is set in the HW register. */)
	{
...
void CPU1_ISR_from_IPC_Channel3(void)
{
	if(1 /* if notify status is set in the HW register. */)
	{
